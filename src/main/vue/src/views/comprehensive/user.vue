  ...
  <div class="pad20">
    ...
    <el-dialog title="添加"
        ...
        <el-form :model="formWarn"
                ...
                <el-form-item label="用户名"
                              prop="userName">
                  <el-input v-model.number="formWarn.userName"
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   size="small"
                   @click="addUser('formWarn')">确 定</el-button>
      </div>
    </el-dialog>
  ...
  data() {
    return {
      ...
      formWarn: {
        ...
        userName: null, // 类型：Number  必有字段  备注：事件类型
        password: null
      },
      // 当前页码
      currentPage: 4,
  ...
  */
   ...
   */
    warnEvent(index, row, txt, num) {
      ...
      vm.formWarn.userName = row.userName
    ...
    addUser(formName) {
          ...
          vm.$refs[formName].validate(valid => {
            if (valid) {
              vm.httpRequst('addUser',formName,'sendFlag','添加成功','添加失败')
            } else {
              return false
            }
          })
